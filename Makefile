...
LIBS_DESC=	install libs and header files
MINIMAL_DESC=	static linked programs, no libs, no extras
...
RUBY_USE=		RUBY=yes
MINIMAL_CONFIGURE_ON=	--enable-static-programs --disable-shared --disable-static
MINIMAL_BUILD_DEPENDS=	${LOCALBASE}/lib/libglib-2.0.a:devel/glib20
MINIMAL_BUILD_DEPENDS+=	${LOCALBASE}/lib/libxml2.a:textproc/libxml2
MINIMAL_PREVENTS=	DEJAVU GRAPH MMAP NLS PERL PYTHON RUBY EXAMPLES DOCS LIBS

post-patch-MINIMAL-on:
	@${REINPLACE_CMD} -E 's|PKGCONFIG --libs|PKGCONFIG --static --libs|' \
		${WRKSRC}/configure
